...
function symlink() {
    ...
    if [ -n "$currentSrc" ]; then

        echo.Yellow "Link already exists for $dst -> $currentSrc"

        if [ "$currentSrc" != "$src" ]; then

            echo.Yellow "New source is different: $src"
            ...
            if [ -e "$src" ]; then
                ...
                echo.Red "New source $src does not exist"
        ...
        if [[ -e "$dst" && -e "$src" ]]; then
            ...
            echo.Yellow "Both Destination: \"$dst\" and Source: \"$src\" exist"
            ...
            echo.Yellow "Both Destination: \"$dst\" and Source: \"$src\" do not exist"
