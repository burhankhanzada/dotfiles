...
class DotfilesTUI:
    ...
    def init_colors(self):
        ...
        curses.init_pair(7, curses.COLOR_BLACK, curses.COLOR_WHITE)  # Inverted cursor highlight
    ...
    def draw(self):
        ...
        for idx, tab in enumerate(self.tabs):
            ...
            tab_str = f" [ {tab['title']} ] "
            if is_active:
                self.safe_addstr(tab_bar_y, col, tab_str, curses.color_pair(6) | curses.A_BOLD)
            else:
                self.safe_addstr(tab_bar_y, col, tab_str, curses.color_pair(5) | curses.A_DIM)
            col += len(tab_str) + 1
        ...
        self.safe_addstr(subtitle_y, 2, tab["description"], curses.color_pair(5) | curses.A_BOLD)
        self.safe_addstr(subtitle_y, max(2, len(tab["description"]) + 4), count_str, curses.color_pair(3) | curses.A_BOLD)
        ...
        for i, row in enumerate(visible_slice):
            ...
            if row["type"] == "category":
                ...
                self.safe_addstr(row_y, 9, f" {label} ", curses.color_pair(1) | curses.A_BOLD)
                ...
                self.safe_addstr(row_y, 2, pointer, pointer_color)
                self.safe_addstr(row_y, 5 + (2 if is_nested else 0), marker, marker_color)
                self.safe_addstr(row_y, 9 + (2 if is_nested else 0), f" {label} ", curses.color_pair(5) | (curses.A_BOLD if is_cursor else 0))
                self.safe_addstr(row_y, 33 + (2 if is_nested else 0), desc, curses.color_pair(5) | curses.A_DIM)

        # 5. Scroll indicators
        if offset > 0:
            self.safe_addstr(list_start_y - 1, min(max_x - 8, 72), " ▲ more", curses.color_pair(3))
        if offset + available_height < len(rows):
            self.safe_addstr(list_start_y + available_height, min(max_x - 8, 72), " ▼ more", curses.color_pair(3))
