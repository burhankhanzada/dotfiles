...
show_package_status() {
    ...
    for pkg in $(discover_available_packages); do
        ...
        case "$pkg" in
            ...
            android-tools)   (command -v android &>/dev/null || command -v adb &>/dev/null || command -v sdkmanager &>/dev/null || command -v avdmanager &>/dev/null) && bin_status="\033[32m✔ installed\033[0m" ;;
